...
```{r}
sources <- str_remove(energy_sources, "\\s*\\(.*\\)") |> str_trim()
```
...
e.  Discussion: Why might regex cleaning be important before performing joins or aggregations?\
    This significantly cleans up the data for further use, esp for labelling and titling in a consistent way. It eliminates extraneous chracters and such as the () and organizes the information in a much more usable manner.
...
states <- states |>
  select(state, Income, Population, Illiteracy, Life.Exp)
# table for regions
...
```{r}
combined_left <- left_join(states, regions)
combined_left
```
...
```{r}
combined_inner <- inner_join(states, regions)
combined_inner
#there isn't a difference, both are 50 
```
...
```{r}
combined_anti <- anti_join(states, regions)
combined_anti
#0, all states match, spelling errors or weird formatting
```

d.  Discussion: Why would you prefer a left join over an inner join when merging state-level datasets?\
    if the left dataset has all of the states then it's preserved, regardless of missing data in the right dataset. inner join would import the errors present in the right dataset
e. Use pivot_longer() to make the socioeconomic data longer.\

```{r}
combined_left <- pivot_longer(combined_left,
    cols = c(Income, Population, Illiteracy, Life.Exp),
    names_to = "variable",
    values_to = "value"
  )
combined_left
...
```{r}
 sales_long <- pivot_longer(sales, 
    cols = c(Jan_Sales, Feb_Sales, Mar_Sales),
    names_to = "Month",
    values_to = "Sales"
  )
  sales_long
...
```{r}
sales_long <- mutate(sales_long, Month = str_remove(Month, pattern = "_Sales"))
sales_long
```
...
```{r}
sales_wide = pivot_wider(sales_long, names_from = "Store", values_from = "Sales")
sales_wide
```
...
```{r}
us_map_joined <- left_join(state_data, us_map, join_by(state == region))
us_map_joined
```
...
b.  Swap Income for Life expectancy to visualize life expectancy instead. Try changing the color scale using scale_fill_gradient() or scale_fill_gradient2()

```{r}
ggplot(us_map_joined, aes(long, lat, group = group, fill = Life.Exp)) +
  geom_polygon(color = "white") +
  coord_fixed(1.3) +
  theme_minimal() +
  scale_fill_viridis_c() +
  labs(title = "Average Life Expectancy by State")
```
